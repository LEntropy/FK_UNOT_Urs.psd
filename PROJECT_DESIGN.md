...
- ~~rust-core C2PA 매니페스트~~ — 부분 완료. `c2pa` 크레이트(v0.89, `rust_native_crypto`) 연동해서 매니페스트 임베딩·커스텀 assertion·콘텐츠해시 무결성 검증까지는 실제로 동작. 다만 **클레임 서명 자체가 재검증 시 통과하지 못하는 이슈를 발견**(`claimSignature.mismatch`) — 우리 키/서명 로직은 독립적으로 정상 확인됐고, 크레이트 내부의 self-check 두 경로가 서로 다른 에러를 내는 걸로 봐서 이 크레이트 버전의 `rust_native_crypto` + 커스텀 Signer 조합 자체의 문제로 추정. 상세 조사 과정과 재현 테스트는 `apps/protection-svc/rust-core/README.md` 참고. **현재로선 소유권 증명은 blockchain-svc가 담당하고, C2PA는 아직 "진짜 암호학적 보증"을 못 준다**

...
- **부분 구현 완료**: (1) Concept Misalignment Layer —
  `apps/protection-svc/ml-engine/src/concept_misalign.py`. VGG19 대신 CLIP
  이미지 임베딩(`open_clip`, ViT-B-32/openai)을 타깃 공간으로 삼아
  `style_cloak.py`와 동일한 epsilon-bounded 최적화 루프를 실행, 이미지
  임베딩을 디코이 컨셉 이미지 쪽으로 끌어당김. `orchestrate.py`의
  `protect()`에 완전 opt-in 파라미터(`concept_misalign_target_path`,
  기본값 없음)로만 연결 — 기존 호출자 동작은 전혀 안 바뀜. **검증 미완료를
  명시적으로 표시**: PHASE4_SCOPING.md §1이 권고한 실제 LoRA 학습 검증
  실험(GPU 필요, 이번 세션엔 GPU 미확보)도, 최소한의 CPU 스모크 테스트조차
  (사전학습 CLIP 체크포인트 다운로드가 이 환경의 외부코드 안전 게이트에
  차단됨) 실행하지 못함 — 코드는 설계된 메커니즘을 그대로 구현하지만 단
  한 번도 실행되지 않은 상태. 상세 내용은 `PHASE4_SCOPING.md` §1과
  `apps/protection-svc/ml-engine/README.md`의 "Concept Misalignment Layer"
  절 참고.
- **부분 구현 완료**: (2) 허니에셋/URL —

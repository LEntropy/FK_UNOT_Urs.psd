"""Scores the multi-surrogate ensemble attack's real LoRA-validation
experiment -- same CLIP-similarity method and verdict logic as
experiments/lora_validation/generate_and_score.py (imports its
generate_samples/clip_similarity rather than duplicating them), scoped to
this experiment's own attacked_v1.safetensors output naming.
...
def main() -> None:
    ...
    print()
    print("=== per-run results (multi-surrogate ensemble attack, epsilon=0.25) ===")
    ...
    print("compare to diffusion_attack (frozen-checkpoint denoising loss, n=10): mean delta +0.0071 (95% CI [-0.0235, +0.0376])")
    print("compare to aspl_attack (single surrogate, epsilon=0.08, n=10): mean delta +0.0050 (95% CI [-0.0094, +0.0195])")
    print("compare to hybrid_attack (style+concept+denoise, epsilon=0.08, n=10): mean delta -0.0054 (95% CI [-0.0255, +0.0146])")

...
# does alternating surrogate-retrain + PGD (the real literature mechanism,
# not diffusion_attack.py's simplified frozen-checkpoint variant) transfer
# into a measurably worse LoRA when someone actually trains on the result?
# See aspl_attack.py's module doc for the mechanism.
#
# Same 5 images x 2 seeds x 2 conditions = 20 trainings as
# diffusion_attack_validation's own run, for a direct, comparable fourth
# data point against style_cloak (+0.0113), concept_misalign (~0), and
# diffusion_attack (+0.0071) -- all three already validated and negative.
#
# Note: like diffusion_attack_validation's step 1/3 (not lora_validation's),
# this experiment's prepare_dataset.py needs kohya_ss's venv (diffusers +
# peft + CUDA) for aspl_attack.py's real surrogate LoRA training + VAE/UNet
# forward/backward.
...
$EXP_DIR     = "$ML_ENGINE\experiments\aspl_validation"
$OUT_DIR     = "$EXP_DIR\out"
...
Run-Step "1/3: preparing datasets (ASPL-attacking all 5 images, kohya venv -- needs GPU+diffusers+peft)" "prepare" {

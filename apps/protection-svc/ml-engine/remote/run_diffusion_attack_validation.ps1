# Diffusion training-loss attack's real LoRA-validation experiment: does
# maximizing the frozen base checkpoint's own denoising loss (instead of a
# VGG19/CLIP proxy) transfer into a measurably worse LoRA when someone
# actually trains on the resulting image? See diffusion_attack.py's module
# doc for the mechanism and its "simplified, no alternating surrogate
# retraining" scope vs full Anti-DreamBooth.
#
# Same 5 images x 2 seeds x 2 conditions = 20 trainings as
# lora_validation's own 2026-07-22 re-validation (n=10), for a direct,
# comparable third data point against style_cloak (+0.0113) and
# concept_misalign (~0, already validated twice and found negative).
#
# Note: unlike run_lora_validation.ps1's step 1/3 (ml-engine's CPU-only
# venv), this experiment's prepare_dataset.py needs kohya_ss's venv
# (diffusers + CUDA) for diffusion_attack.py's real VAE+UNet forward/
# backward -- a genuinely different dependency, not an oversight.
...
$EXP_DIR     = "$ML_ENGINE\experiments\diffusion_attack_validation"
$OUT_DIR     = "$EXP_DIR\out"
...
Run-Step "1/3: preparing datasets (diffusion-attacking all 5 images, kohya venv -- needs GPU+diffusers)" "prepare" {

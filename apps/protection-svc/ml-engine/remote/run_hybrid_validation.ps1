# Hybrid ensemble cloak's real LoRA-validation experiment -- does combining
# every mechanism this project has validated so far (VGG19 style drift,
# CLIP concept drift, ASPL-style surrogate-retrain denoising-loss attack)
# into one joint objective transfer into a measurably worse LoRA, when none
# of the four individually did? See hybrid_attack.py's module doc.
#
# Same 5 images x 2 seeds x 2 conditions = 20 trainings as the prior three
# validation runs, for a direct, comparable fifth data point against
# style_cloak (+0.0113), concept_misalign (~0), diffusion_attack (+0.0071),
# and aspl_attack (+0.0050) -- all four already validated and negative.
#
# Like diffusion_attack_validation and aspl_validation's step 1/3 (not
# lora_validation's), this experiment's prepare_dataset.py needs kohya_ss's
# venv (diffusers + peft + CUDA) for hybrid_attack.py's real surrogate LoRA
# training + VGG19/CLIP feature extraction + VAE/UNet forward/backward.
...
$EXP_DIR     = "$ML_ENGINE\experiments\hybrid_validation"
$OUT_DIR     = "$EXP_DIR\out"
...
Run-Step "1/3: preparing datasets (hybrid-attacking all 5 images, kohya venv -- needs GPU+diffusers+peft)" "prepare" {

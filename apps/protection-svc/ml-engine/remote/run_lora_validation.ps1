...
# Real protection-svc/ml-engine/... nesting synced intact here (not the
# flat C:\dontai-ml-engine\ this project's other GPU experiments use) --
# prepare_dataset.py now imports orchestrate.py (protection-svc/'s own
# module, a level above ml-engine/) to mirror the real pipeline's actual
# eot/perceptual_mask/use_amp choices, which needs that relative structure
...
# Image set itself lives in prepare_dataset.py's IMAGE_CONFIGS (5 as of
# the 2026-07-22 re-validation run, trimmed from the original 10 for GPU
# time -- see that file's own comment) -- this script reads the resulting
# manifest.json rather than hardcoding names here too.
#
# SEEDS reduced from the original [1,2,3] to [1,2] for the same
# GPU-time-feasibility reason -- n=10 (5 images x 2 seeds) instead of the
# original n=30, a real reduced sample re-validating the *current*,
# post-drift preset config (epsilon/color_weight/perceptual_mask/
# clip_transfer_weight/AMP all changed since the original n=30 run -- see
# this experiment's README for the full drift writeup), not a repeat of
# the original run.
$SEEDS       = @(1, 2)
$RUN_NAME    = "v1"
